...
const QuestionCard = () => {
  ...
  const handleSearch = (questionsData) => {
           ...
           {quizStarted && !quizEnded && filteredQuestions && filteredQuestions.length > 0 && currentQuestionIndex < filteredQuestions.length && (
            ...
            {filteredQuestions[currentQuestionIndex]?.correct_answer && (
              <label className="block text-gray-700">
                <input
                  type="checkbox"
                  value={filteredQuestions[currentQuestionIndex]?.correct_answer}
                  checked={selectedAnswer === filteredQuestions[currentQuestionIndex]?.correct_answer}
                  onChange={handleAnswerSelect}
                />
                {filteredQuestions[currentQuestionIndex]?.correct_answer}
              </label>
